...
class SymbolExpressionVisitor(ExpressionVisitor):
    ...
    def to_expression(self, tree, expr):
        ...
        tree.expect(
            explicit_cast(expr.type(), t.type()),
    ...
    def type_cast_expression(expr_node, target_type):
        ...
        element = Tree(
            "expression",
            [expr_node, Tree("to_operator", [Tree("types", [cast_type])])],
        )
...
class ExpressionResolver:
    def __init__(self, module):
        ...
        self.path_resolver = PathResolver(
            symbol_resolver=module.symbol_resolver, module=module
        )
        self.module = module
        self.with_as = False
    ...
    def map(self, tree):
        ...
        for i, item in enumerate(tree.children):
            ...
            key_child.expect(
                new_key.hashable(), "type_key_not_hashable", key=new_key
            )
    ...
    def path_resolve_only_name(self, tree, fn_type):
        ...
        names = tree.children
        tree.expect(
            len(names) == 1, "function_call_invalid_path", fn_type=fn_type
        )
    ...
    def resolve_mutation(self, s, tree):
        ...
        args = self.build_arguments(
            tree.mutation_fragment, name, fn_type="Mutation"
        )
    ...
    def resolve_service(self, tree, output_sym=None):
        ...
        args = self.build_arguments(
            tree.service_fragment, fname=service_name, fn_type="Service"
        )

        if output_sym is not None:
            return self.module.service_typing.resolve_service_output_object(
                tree, service_name, action_name, args, output_sym
            )
    ...
    def check_service_fragment_arguments(self, tree):
        if tree.arguments:
            ...
            tree.expect(
                isinstance(first_arg_name, Token)
                and first_arg_name.type == "NAME",
