...
class Transformer(LarkTransformer):
    ...
    operator_assignments = {
        "ADD_EQUALS": {"name": "PLUS", "op_type": "arith_operator"},
        "SUB_EQUALS": {"name": "DASH", "op_type": "arith_operator"},
        "MUL_EQUALS": {"name": "MULTIPLIER", "op_type": "mul_operator"},
        "DIV_EQUALS": {"name": "BSLASH", "op_type": "mul_operator"},
        "MOD_EQUALS": {"name": "MODULUS", "op_type": "mul_operator"},
    }
    ...
    def assignment(cls, matches):
        ...
        """
        matches[1].expect(
            len(matches[0].children) > 0, "object_destructoring_no_variables"
        )
        token = matches[0].children[0]
        if isinstance(token, Tree):
            matches[0].expect(
                token.data != "inline_expression",
        ...
        if isinstance(c.children[0], Tree):
            ...
            lvalue_expr = Tree("expression", [Tree("entity", [lvalue_path])])
            ...
            op_node = Tree(
                op_assignment["op_type"],
                [
                    assignment_node.create_token(
                        op_assignment["name"], op_token.value[0]
                    )
                ],
            )
            ...
            expr = Tree(
                "expression", [lvalue_expr, op_node, base_expr.children[0]]
            )
    ...
    def service_block(cls, matches):
        ...
        if matches[1].block.rules:
            args = [
                *cls.filter_nested_block(
                    matches[1], ["rules", "block", "arguments"]
                )
            ]
            if len(args) > 0:
                for arg in args:
    ...
    @staticmethod
    def create_when_block(
        service_name, block, command=None, output=None, fragment=None
    ):
        """
        ...
        if fragment:
            return Transformer.create_when_block_tree(
                service_name=service_name, fragment=fragment, block=block
            )
        ...
        return Transformer.create_when_block_tree(
            service_name=service_name, fragment=service_fragment, block=block
        )
    ...
    def create_when_block_tree(service_name, fragment, block):
        ...
        """
        service = Tree("service", [Tree("path", [service_name]), fragment,])
    ...
    def when_block(cls, matches):
        ...
        if when.type == "NAME":
            ...
            return cls.create_when_block(
                service_name=when, output=output, block=matches[-1],
            )
        ...
        if not when.service_fragment.command:
            ...
            if (
                first_arg
                and not isinstance(first_arg.first_child(), Token)
    ...
    def string(self, matches):
        ...
        if (
            matches[0].type == "SINGLE_QUOTED"
            or matches[0].type == "DOUBLE_QUOTED"
        ):
    ...
    def build_inline(node):
        ...
        return Tree(
            ...
            [
                Tree(
                    "entity",
                    [Tree("path", [Tree("inline_expression", [node])])],
                )
            ],
        )
        ...
        if isinstance(matches[0], Token) and matches[0].type == "FLOAT_MUT":
                ...
                [
                    Tree(
                        ...
                        [
                            Tree(
                                "entity",
                                [Tree("values", [Tree("number", [int_tok])])],
                            )
                        ],
                    ),
            ...
            for match in matches[1:]:
                if match.data == "arguments":
                    ...
                    tree = Tree(
                        "mutation", [cls.build_inline(tree), *match.children,]
                    )
        elif len(matches) == 1:
            return matches[0]
        else:
            # normal dot_expressions like "a".mutation()
            tree = Tree("mutation", [matches[0], *matches[1].children])
            for mutation in matches[2:]:
                expression = cls.build_inline(tree)
                tree = Tree("mutation", [expression, *mutation.children,])
